...
# --- count each label ---
count = dataset['tpl_id'].value_counts().sort_values(ascending=1)
...
ds_X = dataset["ticket_msg"]
ds_y = dataset["tpl_id"]
XTrain,XTest,yTrain,yTest = ct.train_test_split(ds_X,ds_y,test_size=0.2,random_state=42)

print('data loaded')

# categories =     # for case categories == None


def size_mb(docs):
    return sum(len(s.encode('utf-8')) for s in docs) / 1e6

data_train_size_mb = size_mb(dataset)
data_test_size_mb = size_mb(dataset)

print("%d documents - %0.3fMB (training set)" % (
    len(XTrain), data_train_size_mb))
print("%d documents - %0.3fMB (test set)" % (
    len(XTest), data_test_size_mb))
